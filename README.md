...
Demo地址: [just-my-socks-mirror.vercel.app](https://just-my-socks-mirror.vercel.app/)

## 项目背景

[Just My Socks](https://justmysocks.net/members/aff.php?aff=18111) 是一个很好用的代理服务提供商，但是官方订阅地址和部分 mirror 有时会出现无法访问的情况，导致你无法更新订阅

这个项目的目的就是提供一个 Vercel HTTP 代理，让你多一条路：

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/ericwyn/just-my-socks-mirror)

部署完成后，请在 Vercel 项目设置中至少配置 `JMS_UPSTREAM_BASE_URL`；如果需要启用安全短链模式，还需要配置 `SEC_KEY`、`SEC_SERVICE`、`SEC_ID`。
...
![首页输入与订阅信息](./screenshot/screenshot1.png)

### 二维码与流量信息展示

![二维码与流量信息展示](./screenshot/screenshot2.png)
...
| `SEC_KEY` | 否 | 短链接访问凭证，配置后才能使用 `/api/sublink`、`/api/counter` 及首页 key 模式，支持多个 key，使用英文逗号分隔，例如 `SEC_KEY=111,222` |
| `SEC_SERVICE` | 否（与 SEC_KEY 配套） | 真实 service 参数，由服务端注入，不对外暴露 |
| `SEC_ID` | 否（与 SEC_KEY 配套） | 真实 id 参数，由服务端注入，不对外暴露 |

如果配置了多个 `SEC_KEY`，任意一个 key 都可以访问同一组 `SEC_SERVICE` / `SEC_ID` 对应的数据。
